...
Интерфейс List реализует абстрактый класс AbstractList, унаследованный от класса AbstractCollection. В свою очередь AbstractList унаследует класс ArrayList И LinkedList.
...
Если требуется быстрая запись данных в контейнер, то лучше воспользоваться LinkedList - связным списком, который наследует AbstractSequentialList реализующий AbstractList. В данном списке хранится ссылка на следующий и предыдущий объекты
коллекции. Данная реализация позволяет быстро выполнять вставку и удаление элементов коллекции, но чтение элемента занимает значительное время.
...
Интерфейс Set реализует инетерфейс Collection. Основной особенностью я вляется то, что множества не содержат дубликаты, а так же имею 
...
Таким же популярным является класс TreeSet. который данные хранит в видел красно-черного дерева в изначально отсортированном виде.. 
